...
4. **Choose what to do with existing data:**
   - _Keep as private_ - everything stays scoped `private` (default). You can promote rows to `team` individually later.
   - _Promote my rows to team_ - all your existing rows become `team`. Use this only if you were a "solo" user who's now joining a team and want everything shared immediately.
