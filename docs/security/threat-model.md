...
_Assumption_: attacker has access to one team member's desktop/device.
...
_Assumption_: a legitimate admin acts in bad faith.
...
_Assumption_: attacker can reach the server's public endpoint.
...
_Assumption_: the upstream Jira/Slack/OpenAI endpoint is malicious or compromised.
...
_Assumption_: an npm package is compromised.
