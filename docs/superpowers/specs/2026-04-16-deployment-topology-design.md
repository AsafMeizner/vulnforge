...
```json
{
  "type": "capabilities",
  "ai": [
    {"name": "team-triage", "task_tags": ["triage","embed"], "provider_type": "ollama-proxied", "available": true}
  ],
  "integrations": [
    {"name": "team-jira", "type": "jira", "actions": ["create_ticket","update_ticket","comment"]}
  ]
}
```
...
1. Spec doc + `todo/` scaffolding + `.gitignore` entry. *(this commit)*
